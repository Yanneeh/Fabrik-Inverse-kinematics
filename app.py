...
class Arm:
    ...
    if np.linalg.norm(self.beginpoint - self.endpoint) < self.armlengte:
        ...
        for i in range(len(self.segments)):
            if i == 0:
              ...
              angle = 180 - getAngle(self.segments[i].v)
              ...
              angle = 180 - math.degrees(math.asin( (np.linalg.norm(self.segments[i].v) * math.sin(math.radians(angleBeteenVector))) / self.segments[i].lengte))
  ...
  def moveTo3D(self, x, y, z):
    ...
    if np.linalg.norm(self.beginpoint - self.endpoint) < self.armlengte:
        ...
        for i in range(len(self.segments)):
            if i == 0:
              # Bereken de hoek tussen de eerste vector in de list en de x as.
              angle = 180 - getAngle(self.segments[i].v, self.beginpoint)
  ...
  def sendAngles(self):
    ...
    if self.ser.is_open:
        print('writing to {}'.format(self.ser.name))
